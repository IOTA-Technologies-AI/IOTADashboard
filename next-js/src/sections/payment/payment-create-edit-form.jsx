...
export function PaymentCreateEditForm({ currentPaymentId }) {
  const router = useRouter();
  const isEdit = !!currentPaymentId;
  ...
  // Fetch outstanding invoices when payment type changes
  useEffect(() => {
  ...
  const fetchPaymentData = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        setFormData({
          paymentType: payment.paymentType || 'AR',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to ${isEdit ? 'update' : 'create'} payment`);
      }
      ...
      setError(genError.message || `Error ${isEdit ? 'updating' : 'creating'} payment`);
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <Button
            type="submit"
            variant="contained"
            disabled={!formData.amount || parseFloat(formData.amount) <= 0 || loading}
          >
