...
export async function GET() {
  try {
    const res = await fetch(`${BASE_URL}/vatconfigs`, {
      method: 'GET',
    ...
    console.error('[Proxy] /api/vatconfigs GET failed:', error);
    return NextResponse.json(
      { vatConfigs: [], message: error.message || 'Failed to fetch VAT configs' },
      { status: 500 }
    );
  }
}
