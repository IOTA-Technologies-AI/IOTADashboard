...
export async function GET(request) {
  try {
    ...
    const url = queryString
      ? `${BASE_URL}/sales/stages?${queryString}`
      : `${BASE_URL}/sales/stages`;
    const res = await fetch(url, { method: 'GET', headers: getHeaders(request) });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch (error) {
    console.error('Proxy /api/sales/stages GET failed', error);
...
export async function POST(request) {
  try {
    ...
    const res = await fetch(`${BASE_URL}/sales/stages`, {
      method: 'POST',
    ...
    console.error('Proxy /api/sales/stages POST failed', error);
