...
export async function GET(_request, { params }) {
  try {
    const res = await fetch(`${BASE_URL}/sales/stages/${params.id}`, {
      method: 'GET',
    ...
    console.error('Proxy /api/sales/stages/[id] GET failed', error);
    return NextResponse.json({ message: 'Failed to fetch stage' }, { status: 500 });
  }
}
...
export async function PATCH(request, { params }) {
  try {
    ...
    console.error('Proxy /api/sales/stages/[id] PATCH failed', error);
...
export async function DELETE(_request, { params }) {
  try {
    ...
    console.error('Proxy /api/sales/stages/[id] DELETE failed', error);
