...
export async function GET(request) {
  try {
    ...
    const url = queryString ? `${BASE_URL}/sales/deals?${queryString}` : `${BASE_URL}/sales/deals`;
    const res = await fetch(url, { method: 'GET', headers: defaultHeaders });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch (error) {
    console.error('Proxy /api/sales/deals GET failed', error);
...
export async function POST(request) {
  try {
    ...
    const res = await fetch(`${BASE_URL}/sales/deals`, {
      method: 'POST',
    ...
    console.error('Proxy /api/sales/deals POST failed', error);
